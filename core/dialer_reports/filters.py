...
class CampaignRecordDateFilter(django_filters.FilterSet):
    ...
    def __init__(self, data=None, *args, **kwargs):
        ...
        # If no data was provided (initial page load)
        if data is None:
            # Create a new mutable QueryDict with default dates
            data['start_date'] = first_day.strftime('%Y-%m-%d')
            data['end_date'] = today.strftime('%Y-%m-%d')
        else:
            # Make a mutable copy if it exists
            data = data.copy()
            data.setdefault('start_date', first_day.strftime('%Y-%m-%d'))
            data.setdefault('end_date', today.strftime('%Y-%m-%d'))

        super().__init__(data=data, *args, **kwargs)


class AdminCampaignRecordDateFilter(django_filters.FilterSet):
    start_date = django_filters.DateFilter(
        field_name='time',
        lookup_expr='gte',
        label='Start Date',
